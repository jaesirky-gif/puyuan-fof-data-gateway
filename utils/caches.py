...
@cache.memoize(timeout=1800, make_name='get_all_funds')
def get_all_funds():
    import pandas as pd
    from surfing.data.wrapper.mysql import ViewDatabaseConnector
    from surfing.data.view.view_models import FundDailyCollection, PFFundDailyCollection
    with ViewDatabaseConnector().managed_session() as mn_session:
        query = mn_session.query(
            PFFundDailyCollection
        )
        df = pd.read_sql(query.statement, query.session.bind)
        df['fund_source'] = 2

        query = mn_session.query(
            *FundDailyCollection.mul_fund_view_to_private_fund_view().keys(),
            FundDailyCollection.order_book_id,
        ).filter()
        dff = pd.read_sql(query.statement, query.session.bind)
        dff = dff.rename(columns=FundDailyCollection.mul_fund_view_to_private_fund_view())
        dff['fund_source'] = 1

        df = df.append(dff)
        df = df.drop(['_update_time'], axis=1)
        df['start_date'] = df['start_date'].apply(lambda x: x.strftime('%Y-%m-%d') if x else None)
        return df
