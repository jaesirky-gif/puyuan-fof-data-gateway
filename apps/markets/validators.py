...
class TimeValidation(BaseValidation):
    ...
    time_para: Literal['ALL','1W','1M','3M','6M','1Y', 'YTD', '3Y', '5Y'] = None


class StockValidation(BaseValidation):
    begin_date: datetime.date = None
    end_date: datetime.date = None
...
class ProductRetValidation(TimeValidation):
    ...
    price_type: Literal['价格线','价格比线'] = '价格线'
...
class AssetInfoValidation(BaseValidation):
    key_words: conlist(str, min_items=1)


class PriceRatioValidation(BaseValidation):
    asset_ids: list
    bmk_id: str
...
class TaaValidation(BaseValidation):
    index_list: list
    index_wgt: list
    frequency: str
    start_date: datetime.date
