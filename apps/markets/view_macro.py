...
from .validators import (TimeValidation, AssetRetValidation, AssetRecentValidation, ProductRetValidation,
                         ProductRecentValidation, ProductCorrValidation, ProductRollingValidation,
                         ValuationTimeValidation, ValuationIndexValidation, ValuationIndustryValidation,
                         ValuationHistoryValidation, StockDebtValidation, SideCapValidation, AHPremValidation, StockValidation)
...
class AssetRetAPI(ApiViewHandler):
    ...
    def post(self):
        ...
        df = PortfolioDataApi().get_asset_price(**data)
        if df is None:
            return
...
class AssetRecentAPI(ApiViewHandler):
    ...
    def post(self):
        ...
        df = PortfolioDataApi().asset_recent_rate(**data)
...
class ProductRecentAPI(ApiViewHandler):
    ...
    def post(self):
        ...
        data_dic = PortfolioDataApi().product_recent_rate_data(data['product_list'])
...
class StockRetAPI(ApiViewHandler):
    ...
    @login_required
    def get(self):
        data = StockValidation.get_valid_data(self.input)
        if not data['time_para'] and (not data['begin_date'] or not data['end_date']):
            raise LogicError('缺少参数')
