...
class PortfolioTradeAPI(ApiViewHandler):
    ...
    def post(self):
        ...
        create_portfolio(g.token.get('manager_id'), data)
        return 'success'

    @login_required
    def put(self):
        data = PortfolioUpdateValidation.get_valid_data(self.input)
        update_portfolio(g.token.get('manager_id'), data)
        return 'success'

    @login_required
    def delete(self):
        data = PortfolioIdValidation.get_valid_data(self.input)
        delete_portfolio(g.token.get('manager_id'), data)
        return 'success'
...
class PortfolioTradeListAPI(ApiViewHandler):
    ...
    @login_required
    def get(self):
        trades = PortfolioTrade.filter_by_query(manager_id=g.token.get('manager_id')).all()
