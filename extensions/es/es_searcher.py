...
from elasticsearch_dsl.query import Match, MatchPhrasePrefix, Regexp, Terms, Term, QueryString
...
class PriFOFSearcher(Searcher):

    def get_usually_query_result(self, key_word, offset, limit, manager_id=None, asset_type=None, is_fof=None):
        must, must_not, should, filters = [], [], [], []
        ss = self.doc_model.search()
        zh_model = re.compile(u'[a-z]')

        if re.compile(u'.*[0-9]{1,}.*').search(key_word):
            should.append(Regexp(fof_id='.*{}'.format(key_word)))
            should.append(Regexp(fof_id='.*{}.*'.format(key_word)))
        elif zh_model.search(key_word):
            should.append(MatchPhrasePrefix(fof_name_pinyin=key_word))
        ...
        must.append(Term(asset_type=asset_type))
        must.append(Term(manager_id=manager_id))
        if is_fof is not None:
            is_fof = '1' if is_fof else '0'
            must.append(Term(is_fof=is_fof))
