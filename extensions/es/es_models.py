from elasticsearch_dsl import Document, Date, Text, Keyword, Boolean
...
class PriFOFFundSearchDoc(Document):

    manager_id = Keyword()
    asset_type = Keyword()
    is_fof = Keyword()
    fof_id = Text(analyzer='ik_max_word')
    fof_id_first = Text(analyzer='standard')
    fof_name = Text(analyzer='ik_max_word')
    fof_name_pinyin = Text(analyzer='pinyin')
    fof_name_first = Text(analyzer='standard')
    update_time = Date()
